...
BaseShader::BaseShader(ID3D11Device* dev, HWND lhwnd)
{
	device = dev;
	hwnd = hwnd;
}
...
void BaseShader::loadVertexShader(const wchar_t* filename)
{
	...
	device->CreateInputLayout(polygonLayout, numElements, vertexShaderBuffer->GetBufferPointer(), vertexShaderBuffer->GetBufferSize(), &layout);
...
void BaseShader::loadHullShader(const wchar_t* filename)
{
	...
	device->CreateHullShader(hullShaderBuffer->GetBufferPointer(), hullShaderBuffer->GetBufferSize(), NULL, &hullShader);
...
void BaseShader::loadDomainShader(const wchar_t* filename)
{
	...
	device->CreateDomainShader(domainShaderBuffer->GetBufferPointer(), domainShaderBuffer->GetBufferSize(), NULL, &domainShader);
...
void BaseShader::loadComputeShader(const wchar_t* filename)
{
	...
	// Create the domain shader from the buffer.
	device->CreateComputeShader(computeShaderBuffer->GetBufferPointer(), computeShaderBuffer->GetBufferSize(), NULL, &computeShader);
